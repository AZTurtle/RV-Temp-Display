...
static const String SENSORPUSH_API_BASE = "https://api.sensorpush.com/api/v1/";
...
static SensorPushApi sensorPushApi;
static TaskHandle_t vUpdateSensorPushTaskHandler = NULL;
TaskHandle_t vGraphTaskHandler = NULL;
                       ...
                       uint8_t buttonState)
{
    ...
    case AceButton::kEventLongPressed:
        ...
        if (vUpdateSensorPushTaskHandler) {
            vTaskDelete(vUpdateSensorPushTaskHandler);
        }
        if (vGraphTaskHandler) {
            vTaskDelete(vGraphTaskHandler);
        }
...
void setup()
{
    ...
    if(!file){
        Serial.printf("Couldn't find file: %s | Making new file", CONFIG_PATH);
        //Failed to open file... make new one

        createDefaultConfig();
    } else {
        root = getJSONFromFile(file);

        if(root) {
            cJSON* ssid_ = cJSON_GetObjectItem(root, "ssid");
            if(!ssid_) {
                Serial.println("Failed to get SSID");
            } else {
                ssid = ssid_->valuestring;
                WiFi_SSID = ssid_->valuestring;
            }

            cJSON* passwd_ = cJSON_GetObjectItem(root, "password");
            if(!passwd_) {
                Serial.println("Failed to get password");
            } else {
                password = passwd_->valuestring;
            }

            cJSON* email = cJSON_GetObjectItem(root, "sp_email");
            if(!email) {
                Serial.println("Failed to get sp_email");
            } else {
                sensorpushEmail = email->valuestring;
            }

            cJSON* sp_passwd = cJSON_GetObjectItem(root, "sp_password");
            if(!sp_passwd) {
                Serial.println("Failed to get sp_password");
            } else {
                sensorpushPsswd = sp_passwd->valuestring;
            }
...
void WiFiEvent(WiFiEvent_t event)
{
    ...
    case ARDUINO_EVENT_WIFI_SCAN_DONE:
        ...
        lv_msg_send(WIFI_SCAN_DONE, NULL);
        break;
    ...
    case ARDUINO_EVENT_WIFI_STA_CONNECTED:
        ...
        lv_msg_send(WIFI_CONNECTED, NULL);
        break;
    ...
    case ARDUINO_EVENT_WIFI_STA_GOT_IP:
        ...
        if (!vUpdateSensorPushTaskHandler) {
            xTaskCreate(updateSensorPushTask, "sensorpush", 30 * 1024, NULL, 9, &vUpdateSensorPushTaskHandler);
        }
        lv_msg_send(WIFI_MSG_ID, NULL);
        break;
...
const String sensorURL = SENSORPUSH_API_BASE + "devices/sensors/";
const String sampleURL = SENSORPUSH_API_BASE + "samples/";
const String authURL   = SENSORPUSH_API_BASE + "oauth/authorize/";
const String accessURL = SENSORPUSH_API_BASE + "oauth/accesstoken/";
static String accesstoken = "";

int getAccessToken(String email, String password) {
    ...
    httpBody = "";
     ...
     if (https.begin(client, authURL)) {
        ...
        httpCode = https.POST(
        "{\n"
            "\"email\": \"" + sensorpushEmail + "\",\n"
            "\"password\": \"" + sensorpushPsswd + "\"\n"
        "}");
        ...
        if (httpCode > 0) {
            ...
            if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
                ...
                cJSON *code = cJSON_GetObjectItem(root, "authorization");
                if (code) {
                    authCode = code->valuestring;
                }
                cJSON_Delete(root);
            }
        } else {
    ...
    if (!authCode.isEmpty() && https.begin(client, accessURL)) {
        https.addHeader("accept", "application/json");
        https.addHeader("Content-Type", "application/json");
...
void updateSensorPushTask(void *ptr){
    ...
    while (1) {
        ...
        if (xSemaphoreTake(xWiFiLock, portMAX_DELAY) == pdTRUE) {
            ...
            client.setCACert(SensorPushRootCA);

            
            Serial.println(ESP.getFreeHeap());
            httpCode = getAccessToken(sensorpushEmail, sensorpushPsswd);
            if(accesstoken.isEmpty()){
                Serial.println("Can't get access token");
            } else {
                //SENSOR INFO
                if (https.begin(client, sensorURL)) {
                    https.addHeader("accept", "application/json");
                    https.addHeader("Authorization", accesstoken);
                ...
                //SAMPLE DATA
                if (https.begin(client, sampleURL)) {
                    https.addHeader("accept", "application/json");
                    https.addHeader("Authorization", accesstoken);

                    httpCode = https.POST(
                    "{\n"
                        "\"limit\": " + String(GRAPH_RES) + "\n"
                    "}");
                    
                    if (httpCode > 0) {
                        Serial.println("[HTTPS] POST... code: " + String(httpCode));
                        if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
                            sampleData = https.getString();
                        }
                    } else {
                        Serial.println("[HTTPS] POST... failed, error: " + https.errorToString(httpCode));
                    }
            ...
            while (1) {
                ...
                root = cJSON_Parse(sensorInfo.c_str());
                if (!root) {
                    Serial.println("GET SENSOR INFO FAILED");
                    break;
                }

                samples = cJSON_Parse(sampleData.c_str());
                if (!samples) {
                    Serial.println("GET SAMPLES FAILED");
                    break;
                }
                cJSON* data = cJSON_GetObjectItem(samples, "sensors");
                if (!data){
                    Serial.println("GET DATA FAILED");
                    break;
                }
                cJSON* date = cJSON_GetObjectItem(samples, "last_time");
                if (!date){
                    Serial.println("GET DATE FAILED");
                    break;
                }

                sensorPushApi.last_time = date->valuestring;
                ...
                for (int i = 0; i < len; i++){
                    ...
                    if (!item) {
                        Serial.println("GET ITEM FAILED");
                        continue;
                    }
                    cJSON* _name = cJSON_GetObjectItem(item, "name");
                    if (!_name) {
                        Serial.println("GET NAME FAILED");
                        continue;
                    }

                    const char* name = _name->valuestring;
                    bool found = false;
                    int index = 0;

                    for (int i = 0; i < SENSOR_NUM; i++){
                        if (strcmp(name, sensorPushApi.sensors[i].name) == 0) {
                            found = true;
                            index = i;

                            cJSON* batt = cJSON_GetObjectItem(item, "battery_voltage");
                            if (!batt) {
                                Serial.println("GET BATTERY VOLTAGE FAILED");
                                continue;
                            }

                            sensorPushApi.sensors[index].batt = batt->valuedouble;
                            break;
                        }
                    }
                    if (!found) {
                        Serial.println("Couldn't find " + String(name));
                        continue;
                    }

                    cJSON* sensorData = cJSON_GetObjectItem(data, item->string);
                    if (!sensorData) {
                        Serial.println("GET SENSOR_DATA FAILED");
                        continue;
                    }

                    int len = cJSON_GetArraySize(sensorData);

                    for(int i = 0; i < len; i++){
                        cJSON* values = cJSON_GetArrayItem(sensorData, i);
                        if (!values) {
                            Serial.println("GET VALUES FAILED");
                            continue;
                        }

                        cJSON* temperature = cJSON_GetObjectItem(values, "temperature");
                        cJSON* humidity = cJSON_GetObjectItem(values, "humidity");

                        if(temperature && humidity) {
                            sensorPushApi.sensors[index].sample[i].temp = temperature->valuedouble;
                            sensorPushApi.sensors[index].sample[i].humidity = humidity->valuedouble;
                        }
                    }
                }

                lv_msg_send(SENSORPUSH_MSG_ID, &sensorPushApi);
                done = true;
                break;
            }

            if (root) cJSON_Delete(root);
            if (samples) cJSON_Delete(samples);
            xSemaphoreGive( xWiFiLock );
        }

        delay(done ? 60000 : 60000);
    }
}